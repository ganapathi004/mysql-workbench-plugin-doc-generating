...
def documentation(diagram):
    ...
    for figure in diagram.figures:
        if hasattr(figure, "table") and figure.table:
            text += "* [__" + figure.table.name + "__](#" + figure.table.name + ")\n"
...
def writeTableDoc(table):
    text = "## __`" + table.name + "`__\n\n"

    text += table.comment + "\n\n"

    text += "### __`" + table.name + " columns`__\n\n"
...
def writeColumnDoc(column, table):
    ...
    if column.autoIncrement == 1:
        attributes.append("Auto increments")
    ...
    for fk in table.foreignKeys:
        if fk.columns[0].name == column.name:
            text +=  ("<br /><br />" if column.comment else "") + "foreign key to column `" + fk.referencedColumns[0].name + "` on table [" + fk.referencedColumns[0].owner.name + "](#" + fk.referencedColumns[0].owner.name + ")."
            break
